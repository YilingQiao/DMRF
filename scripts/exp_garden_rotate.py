...
if __name__ == "__main__":
    ...
    testbed.init_rt(config_path="./scripts/exp/garden_rotate.json")
    # testbed.init_rt(config_path="./scripts/exp/garden_arrow.json")
    ...
    # view_dir_1 = np.array([0.611,-0.577,-0.542])
    testbed.background_color = [0.843, 0.843, 0.843, 1.000]
    testbed.rt_set_shadow_decay(1)
    view_dir_1 = np.array([0.570,-0.667,-0.480])
    view_dir_2 = np.array([0.564,-0.635,-0.528])  
    # testbed.view_dir = np.array([0.351,0.658,1.594])  
    # testbed.look_at = [0.302,0.331,0.084]
    testbed.look_at =  [0.206,0.773,0.248]
    half_circle = 40    
    testbed.view_dir = view_dir_1
    testbed.scale = 1.500

    testbed.rt_set_light_falloff_start_angle(0.08)
    testbed.rt_set_light_falloff_end_angle(0.12)
    testbed.rt_set_light_albedo(np.array([5., 5., 5.]))
    testbed.rt_set_light_lookat(np.array([0., -1., 0.]))
    ...
    if not args.gui:
        ...
        for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
            ...
            # exposure
            p = 40 / np.pi
            testbed.exposure = np.cos(i/p) - 2
            # testbed.rt_set_light_lookat(np.array([np.cos(i/10)*0.1, -1, np.sin(i/10)*0.1]))
            # testbed.rt_set_light_albedo(np.array([np.cos(i/10)*5, np.cos(i/10)*5, np.cos(i/10)*5]))
            
            # vie direction
            y = -0.4
            view = np.array([np.cos(i/p)*np.sqrt(1-y**2), y, np.sin(i/p)*np.sqrt(1-y**2)])
            testbed.view_dir = view



            # lightsrc 
            r = 0.3
            testbed.rt_set_light_lookat(np.array([np.cos(i/p)*r, -1, np.sin(i/p)*r]))
            # testbed.rt_set_light_albedo(np.array([np.cos(i), np.cos(i), np.cos(i)]))

            # obj
            robj = 1.
            center = np.array([np.cos(i/p)*robj + 0.5, 0.8, np.sin(i/p)*robj + 0.5])
            testbed.rt_set_hittable_center(0, center)
    ...
    # python ./scripts/exp_garden_rotate.py --mode nerf --load_snapshot ./extra_data/nerf360/garden/35000.msgpack  --width 800 --height 600 --video_spp 1
    render_frame = 0
    tqdm_last_update = 0
    if n_steps > 0:
        with tqdm(desc="Training", total=n_steps, unit="step") as t:
            while testbed.frame():
                ...
                y = -0.4
                p = 40 * np.pi
                view = np.array([np.cos(render_frame/p)*np.sqrt(1-y**2), y, np.sin(render_frame/p)*np.sqrt(1-y**2)])
                testbed.view_dir = view
                # testbed.view_dir =    view_circle(view_dir_1, view_dir_2, render_frame, half_circle)
                # testbed.exposure = np.sin(render_frame/20)
                


                r = 0.3
                testbed.rt_set_light_lookat(np.array([np.cos(render_frame/20)*r, -1, np.sin(render_frame/20)*r]))
                testbed.rt_set_light_albedo(np.array([np.cos(render_frame), np.cos(render_frame), np.cos(render_frame)]))
                
                robj = 1.
                center = np.array([np.cos(render_frame/20)*robj + 0.8, 0.5, np.sin(render_frame/20)*robj + 0.5])
                testbed.rt_set_hittable_center(0, center)
