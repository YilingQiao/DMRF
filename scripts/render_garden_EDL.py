...
from edl_utils import load_event_data, get_global_lights, modify_config_for_timestep
...
if __name__ == "__main__":
    ...
    BASE_CONFIG_DIR = "./scripts/exp"
    BASE_CONFIG_FILENAME = "garden_rotate.json"

    testbed = ngp.Testbed(mode)
    if args.gui:
        testbed.init_rt(config_path=os.path.join(BASE_CONFIG_DIR, BASE_CONFIG_FILENAME))
    ...
    testbed.view_dir = [0.858,-0.510,0.065]
    testbed.look_at = [0.302,0.331,0.084]
    testbed.scale = 2
    testbed.tonemap_curve = ngp.TonemapCurve.ACES
    # ========================================================================================

    testbed.rt_set_Equirectangular(True)
    # python ./scripts/render_garden_EDL.py --mode nerf --load_snapshot ./extra_data/nerf360/garden/35000.msgpack  --width 4096 --height 2048 --video_spp 4
    ...
    if not args.gui:
        ...
        for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
            # original_config_path = os.path.join(BASE_CONFIG_DIR, BASE_CONFIG_FILENAME)
            # tmp_config_path = os.path.join(BASE_CONFIG_DIR, "tmp", BASE_CONFIG_FILENAME)

            # modify base config's OBJ filepaths for the current frame
            # current_frame_idx = i + 1
            # modify_config_for_timestep(original_config_path, tmp_config_path, current_frame_idx)

            # re-initialize scene with current mesh elements
            # print(f"Loading current frame {current_frame_idx} mesh elements...")
            # testbed.init_rt(config_path=original_config_path)
