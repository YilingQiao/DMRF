...
from edl_utils import load_event_data, get_global_lights
...
if __name__ == "__main__":
    ...
    testbed.init_rt(config_path="./scripts/exp/classroom.json")
    ...
    fps = 30
    n_seconds = 90
    testbed.background_color = [0.0, 0.0, 0.0, 1.000]
    testbed.rt_set_shadow_decay(1)
    events_filepath = "./data/autolighting/classroom/events.json"

    # Camera settings
    testbed.exposure = 0.6
    testbed.view_dir = [0.981, -0.026, 0.191]
    testbed.look_at = [0.588, 0.960, 0.899]
    testbed.scale = 0.36
    # ========================================================================================

    testbed.rt_set_light_falloff_start_angle(0.16)
    testbed.rt_set_light_falloff_end_angle(0.2)
    testbed.rt_set_light_albedo(np.array([10., 10., 10.]))
    testbed.rt_set_light_lookat(np.array([0., -1., 0.]))

    # events = load_event_data(events_json_path=events_filepath)
    # event_light_sources = get_global_lights(camera_position=None, scene_events=events) # TODO AGAO: get camera position
    ...
    if not args.gui:
        ...
        for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
            testbed.rt_set_light_lookat(np.array([0.0, -1.0, 0.0]))
