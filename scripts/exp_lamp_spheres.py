...
if __name__ == "__main__":
    ...
    if not args.gui:
        ...
        start_view_dir = [0.110, -0.652, 0.846]
        start_look_at = [0.5, 0.4, 0.5]
        start_scale = 0.7

        end_view_dir = start_view_dir
        ...
        testbed.scale = interpolate_scalar(start_scale, end_scale, start_frame, n_frames)
        testbed.view_dir = interpolate_vec3(start_view_dir, end_view_dir, start_frame, n_frames)
        testbed.look_at = interpolate_vec3(start_look_at, end_look_at, start_frame, n_frames)

        frame = testbed.render(resolution[0], resolution[1], args.video_spp, True,
                               0.0/n_frames, 1.0/n_frames, args.video_fps,
                               shutter_fraction=0.5)
        write_image(f"tmp/{start_frame}.jpg", frame, quality=100)
        exit()
    # end video
